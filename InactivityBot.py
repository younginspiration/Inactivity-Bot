...
def check_user_activity(self, username: str, user_groups: List[str]) -> None:
    ...
    if days_inactive >= self.RIGHTS_REMOVAL_THRESHOLD:
        ...
        # Add monitored rights to rights to remove
    ...
    # Warning check (only for users with monitored rights who haven't yet reached removal threshold)
    elif any(right in self.BOT_REMOVABLE_RIGHTS for right in user_groups) and days_inactive >= self.WARNING_THRESHOLD:
